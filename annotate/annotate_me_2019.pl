...
Needed for HHR
     ...
     -cluster_members   => TSV file of HHblits database cluster members
...
pod2usage $! unless &GetOptions(
 ...
 # for HHBlits:
  'cluster_members:s' => \$hhblits_cluster_tsv_file,
);
...
sub process_protein_hhr() {
 ...
 die "HHR output processing requires a HHblits cluster tsv file\n" unless $hhblits_cluster_tsv_file && -s $hhblits_cluster_tsv_file;
...
#we currently won't parse it on the jamp
   ...
   # since 2018 or so they no longer store the accessions in the description.
	...
	foreach my $id ( @{$cluster_identities_hashref -> { $accession } } ){
	   #it's a hash because the old code needed and didn't want to change too much
          $hits{$id}  = 1;
          $check{$id} = 1;
	}
