...
local double f_goal;
local double (*pfunction)(double x, void *params);

/* it turns out C++ already defines 'terminate' - bc 040826 */
local wn_bool lo_terminate;
...
local wn_bool x0_is_in_legal_range(double x0)
{
  if(num_low_data > 1)
  {
    if(!(xs[-1] < x0))
...
local void gen_quadratic_probe(wn_bool *psuccess,double *pxnew)
{
  int i;
...
local wn_bool pos_greater_found(void)
{
  int i;
...
local wn_bool neg_greater_found(void)
{
  int i;
...
local wn_bool optimum_found(void)
{
...
(
  ...
  double (*passed_pfunction)(double x, void *params),
...
)
{
       ...
       )
  {
    ...
    */
    ...
    fnew = wn_clip_f((*pfunction)(xnew, params));
...
(
  ...
  double *pval_min,
  double *px_min,
  double (*pfunction)(double x, void *params),
  int max_improve_iterations,
  void *params
)
{
  ...
  f0 = wn_clip_f((*pfunction)(x0, params));
  f1 = wn_clip_f((*pfunction)(x1, params));
  f2 = wn_clip_f((*pfunction)(x2, params));

  wn_minimize_1d_raw(pcode,&f0,&f1,&f2,&x0,&x1,&x2,f1,
		     pfunction,
		     max_improve_iterations,WN_IHUGE, params);
