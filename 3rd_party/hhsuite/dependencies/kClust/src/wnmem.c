...
DEBUGGING:
  ...
  wn_bool wn_mem_in_group(p,group)
...
local wn_bool initialized = FALSE;
...
{
  static wn_bool first_time = TRUE;
  char *pfn;
...
local wn_bool group_is_in_group_list(wn_memgp group)
{
  wn_memgp g;
...
local wn_bool memory_is_in_block(ptr p,wn_mem_block block)
{
...
wn_bool wn_mem_in_group(ptr p,wn_memgp group)
{
  wn_mem_block block;
...
void wn_abandon_memory_leaks(wn_bool abandon_default_group)
{
  /* stub */
}
