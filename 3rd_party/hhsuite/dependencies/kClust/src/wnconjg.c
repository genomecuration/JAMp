...
extern wn_bool wn_force_optimize_stop_flag;
...
local wn_bool vects_equal(double v1[],double v2[],int len)
{
  int i;
...
local wn_bool parabola_fit_improvement_wrong(double y1,double y0,double b,
					  double tolerance)
{
...
)
{
  ...
  *pval = (*pfunction)(buffer_vect, params);
}
...
local double simple_eval_function(double x, void *params)
{
  double ret;

  eval_function(&ret,save_vect,save_direction,x,save_len,save_pfunction, params);

  return(ret);
}


local wn_bool x0_too_far_out(double x0,double x2,double threshold)
{
...
local wn_bool x0_too_far_in(double x0,double x2,double threshold)
{
  wn_assert(threshold < 1.0);
...
(
  int *pcode,
  wn_bool *pfirst_parabolic_fit_succeeded,
  double *pval,
  double *pjump_len,
  double vect[],
  double direction[],
...
)
{
  ...
  *pcode = WN_SUCCESS;
  ...
  eval_function(&y2,vect,direction,x2,len,pfunction, params);
  ...
  if(code != WN_SUCCESS)
  {
    ...
    */
    ...
    eval_function(&y0,vect,direction,x0,len,pfunction, params);
...
(
  ...
  int len,
  double (*pfunction)(double vect[], void *params),
  void (*pgradient)(double grad[],double vect[], void *params),
  int max_iterations,
  void *params
)
{
  ...
  wn_bool function_free_method,last_was_function_free_method;
  wn_bool first_parabolic_fit_succeeded;
  ...
  val = (*pfunction)(vect, params);
  (*pgradient)(g,vect, params);
  ...
  for(iteration=0;;++iteration)
  {
    ...
    if(function_free_method)
    {
      ...
      */
      ...
      (*pgradient)(g,buffer_vect, params);
	...
	*/
    ...
    else  /* function based method */
    {
      ...
      line_minimize(pcode,&first_parabolic_fit_succeeded,&val,&jump_len,
		    vect,
		    direction,last_val,last_g,last_jump_len,len,pfunction, params);
      if(*pcode != WN_SUCCESS)
      {
        wn_gppop();
        wn_gpfree();
