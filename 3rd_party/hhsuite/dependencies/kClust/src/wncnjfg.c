...
/* #define INFINITY  WN_FHUGE ** not used, redefinition sometimes */
...
wn_bool wn_force_optimize_stop_flag;
...
typedef struct slow_linesearch_type_struct
{
  ...
  wn_bool isreal0,isreal1,isreal2;  /* point has been initialized */
  wn_bool quadconv1;

  /* number of minimums thrown away while shrinking bracketing interval */
  int min_lost;
  int bisection_count;   /* a robust probe counter */
  int suspect_bisection; /* number of suspicious bisections */

  /* start point for slow linesearch, use for termination tests */
  double fstart,dfstart;   
...
typedef struct context_struct
{
  ...
  wn_bool (*pterminate)(double vect[],double best_f,int iteration);
  ...
  double xls;
  double x0s_init;
  wn_bool isreal0,isreal0opt;   /* is real data, not interpolated */
  wn_bool isreal1;
  wn_bool isrealls;
  double norm2_grad0,norm2_grad1,norm2_grad0opt;
  double norm2_dir0,norm2_dir1;
  double dot_grad0opt_dir0,dot_grad0_dir1,dot_grad0opt_grad0,dot_grad0_grad1;
...
local wn_bool close_to_each_other(double a, double b, double margin)
{
  double absa, absb;
...
local wn_bool is_not_NaN(double x)
{
  return(x <= 0.0 || x >= 0.0);
}
...
/*
  A real data point is declared unreasonable if
  its function value or derivative value are not valid numbers
*/
local wn_bool real_unreasonable_point
(
 wn_bool is_real,
 double f,
 double df
)
{
  if(is_real)
  {
    if(!is_valid_number(f))
    {
      return(TRUE);
    }
    else if(!is_valid_number(df))
    {
      return(TRUE);
    }
  }
...
{
  /* double rnd; ** unused - bchapman 041111 */
  int sign;
...
local void forward_shift_state(cntxt c)
{
  ...
  c->dot_grad0_grad1 = c->dot_grad0opt_grad0;
...
local void backward_shift_state(cntxt c)
{
  ...
  c->dot_grad0opt_grad0 = c->dot_grad0_grad1;
...
local wn_bool default_terminate(double vect[],double best_f,int iteration)
{
  return(FALSE);
}
...
local void do_first_funcgrad_call(cntxt c)
{
  ...
  */
  ...
  wn_assert(is_not_NaN(c->norm2_grad0));
  ...
  */
  ...
  else if (c->jump_len_flag == WN_CONJ_MUL_FACTOR) 
  {
    /* take a multiple of the gradient */
    c->x1opt = c->first_step_len;
  } 
  else if (c->jump_len_flag == WN_CONJ_CURVATURE)
  {
    /* use first_step_len as a estimate of curvature c --
     * step = (1/c) gradient since the step should solve 
     * step = (H^-1) g == (1/curvature) g
     */
    c->x1opt = 1./c->first_step_len;
  } 
  else if (c->jump_len_flag == WN_CONJ_DISTANCE)
  {
    /* scale step len to move each coord first_step_len (on average) */
    c->x1opt = c->first_step_len*sqrt(c->num_vars/c->norm2_dir0);
  } 
  else 
  {
...
)
{
  ...
  {
    *pyout = (y1*(xout-x2) + y2*(x1-xout))/denom;
  }
}
...
)
{
  ...
  /* doing the vector interpolation at the same time as the dot and
     the norm saves on memory access time, making the dot and the
     norm much less expensive */
  if(denom == 0.0)
  {
    wn_add_scaled_vects_and_dot_and_norm_par(c->pc,gopt,
                            pnorm2_gopt,
                            pdot_gopt_g1,
			    g1,0.5,
			    g2,0.5,
...
(
  wn_bool *psuccess,double *pa,double *px0,double *pb,
...
local wn_bool accuracy_sufficient(double b,double f1,double f2)
{
  double fmax;
  double d,last_d;
  int i;
...
#if 0
local void min_of_cubic(wn_bool *psuccess,double *pb,double *pxmin,
			double coef[4])
{
  double d_coef[3];
  double discriminant;
			...
			double coef[4])
{
  ...
  double dd_coef[2];
  double discriminant;
  /* double remainder; ** unused - bchapman 041111 */
...
(
  wn_bool *psuccess,double *px0,double *pb,
  double x1,double f1,double df1,
  double x2,double f2,double df2
)
{
...
local void set_jumplen(cntxt c)
{
  ...
  if(
     (c->x1opt != 0)
     &&
     (wn_abs(c->x0s) > MAX_STEP_LENGTH_INCREASE*wn_abs(c->x1opt))
    )
  {
    c->x0s = c->x1opt;
#if DEBUG
...
*/
local wn_bool wolfe_condition_satisfied(double x0,double f0,double df0,
...
local wn_bool opt_improvement_adequate
(
...
*/
local wn_bool gradient_interpolation_error_small_enough
(
...
local void determine_how_far_to_backout
(
 cntxt c,
 wn_bool *psuccess0,
 wn_bool *psuccess1
  ...
  else  /* !(c->isreal0) */
  {
    ...
    if(
        ...
        */
        ...
        /*
          &&
        (wn_abs(c->dot_grad0_dir1)/sqrt(c->norm2_dir1)/sqrt(c->norm2_grad0) <
	.25)
       */
      )
    {
...
/*
  ...
  more carefully.
*/
local void do_fast_linesearch0(cntxt c, wn_bool *psuccess0, wn_bool *psuccess1)
{
  wn_bool success;
...
#   endif
    ...
    )
  {
    ...
    */
    ...
    if(wn_abs(c->dot_grad0opt_dir0) <= MAX_COS_VALUE * 
       sqrt(c->norm2_dir0 * c->norm2_grad0opt))
    {
...
#   endif
  ...
  c->norm2_grad0opt = wn_norm2_vect_par(c->pc, c->grad0opt, c->num_vars);
  c->dot_grad0opt_grad0 = wn_dot_vects_par(c->pc, c->grad0opt, c->grad0, 
                                           c->num_vars);
  c->dot_grad0opt_dir0 = 
...
#   endif
  ...
  if(
      opt_improvement_adequate(c->x0opt,c->f0opt,c->dot_grad0opt_dir0,
			       ...
			       0.75)
	&&
      (wn_abs(c->dot_grad0opt_dir0) <= MAX_COS_VALUE *
       sqrt(c->norm2_dir0 * c->norm2_grad0opt))
    )
  {
#   if DEBUG
...
#if DEBUG
local wn_bool
doubles_same(double x, double ref)
{
  /* is x equal to ref to within numerical error ? */
  return wn_abs(x - ref) <= 1e-10 * wn_abs(ref);
}
#endif
...
#if DEBUG
  if(!(doubles_same(c->norm2_grad0, 
		    wn_norm2_vect_par(c->pc, c->grad0, c->num_vars))))
  {
    printf("WARNING: %lg != %lg\n",
	   c->norm2_grad0, wn_norm2_vect_par(c->pc, c->grad0, c->num_vars));
  }
  if(!(doubles_same(c->dot_grad0_grad1, 
		    wn_dot_vects_par(c->pc, c->grad0, c->grad1, c->num_vars))))
  {
    printf("WARNING: %lg != %lg\n",
	   c->dot_grad0_grad1, 
	   wn_dot_vects_par(c->pc, c->grad0, c->grad1, c->num_vars));
    ...
    */
      ...
      )
    {
      ...
      */
      ...
      calc_conj_weights(&success,&curvature,&wdir1,
		        c->x1opt,0.0,
		        c->norm2_grad0,c->norm2_grad1,c->dot_grad0_grad1,
...
#   endif
  ...
  else if(c->no_reset_run_len > 0)
  {
    ...
    {
#     if DEBUG
        printf("Resetting because no_reset_run_len > 0 and iteration >= reset_at_iteration)\n");
        printf("reset !!!!!!!!!!!!!!!!!!!!!!!!!\n");
#     endif
      wdir1 = 0.0;
      c->no_reset_run_len =
	(int)(c->no_reset_run_len * c->reset_expand_factor);
      c->reset_at_iteration = c->iteration+c->no_reset_run_len;
      c->iteration_of_last_reset = c->iteration+1;
    }
  }
...
local wn_bool points_are_convex(double x0,double f0,double df0,
			     double x1,double f1,double df1)
{
  wn_assert(x0 != x1);
...
local wn_bool min_is_bracketed(double x0,double f0,double df0,
			    double x1,double f1,double df1)
{
  wn_assert(x0 != x1);
...
local wn_bool insert_adds_info
(
  cntxt c,
  double xnew,
  double fnew,
...
)
{
  ...
  *psubstantial_change = FALSE;
  ...
  /*
    If the new point is best but outside the present bracketing interval,
    then it gets preferential treatment
  */
  if(cmp  < 0)
  {
    /* Check if the best point is to the left of the bracketing interval */
    if(
	c->sls.isreal0
	 &&
	(x < c->sls.x0)
	...
	we have three points and the best f value comes from outside
    ...
    /* Check if the best point is to the right of the bracketing interval */
    else if(
	     c->sls.isreal2
	       &&
	     (x > c->sls.x2)
...
#if 0
local wn_bool bisection_close_to_parabola
(
...
local wn_bool robust_close_to_parabola(cntxt c,
...
local wn_bool test_quadconv_point(cntxt c,double f,double df)
{
  return(
...
*/
local wn_bool test_quadconv(cntxt c,double f0,double df0,double f1,double df1)
{
  return(
...
local void do_slow_linesearch(cntxt c)
{
  ...
  wn_bool robust;
  wn_bool quadconv;
  wn_bool substantial_change;
  wn_bool test_bisection;
  double bdf1, bdf2;
...
#   endif
    ...
    /* If we have unreasonable real data points then do robust probe */
    if(!robust)
    {
      if(
         real_unreasonable_point(c->sls.isreal0, c->sls.f0, c->sls.df0)
         ||
         real_unreasonable_point(c->sls.isreal1, c->sls.f1, c->sls.df1)
         ||
         real_unreasonable_point(c->sls.isreal2, c->sls.f2, c->sls.df2)
        )
        {
#if DEBUG
          printf("Unreasonable real data point detected -> robust probe.\n");
#endif
          robust = TRUE;
        }
    }
...
local wn_bool termination_condition(cntxt c)
{
...
local int solution_cmp(wn_bool isreal1,double f1,
		       wn_bool isreal2,double f2)
{
...
#if DEBUG
  /* Check returned minimum value */
  eval_min = wn_clip_f((*(c->pfuncgrad))(c->grad0, c->vect));
  if(eval_min != *pval_min)
  {
    /*
      This used to be an assert. It got replaced with a warning due to
      small noise occasionally encountered in the CGPL delay objective
    */
    printf("WARNING: while checking the returned min, %20.20lg != %20.20lg\n",
	   eval_min, *pval_min);
  }
#endif
...
)
{
  ...
  wn_assert((first_step_len > 0.0) && (first_step_len < WN_FHUGE));

  c->first_step_len = first_step_len;
  c->jump_len_flag = jump_len_flag;
  ...
  for(c->iteration=0;;++(c->iteration))
  {
    ...
    printf("After %d iterations, f = %lg\n", c->iteration, c->best_f);
    printf("\n");
#endif
...
#if DEBUG
    printf("Iterations since (best f = %lg) = %d\n",
           c->best_f, c->iters_since_best_f0);
    printf("\n");
#endif
  }
...
)
{
  wn_conj_funcgrad_method_parallel(pcode, pval_min, vect, num_vars, pfuncgrad,
			      	   pterminate, first_step_len, jump_len_flag,
