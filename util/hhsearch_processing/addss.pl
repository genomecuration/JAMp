...
if ($informat eq "hmm" && !$outfile) {
    die("Error: no output file given. With the -hmm option an output file is obligatory\n");
}
...
if ($informat ne "hmm") {
    ...
    if ($v>=1) {warn ("Filtering alignment to diversity $neff ...\n");}
    ...
    if ($v>=2) {warn("Predicting secondary structure with PSIPRED ... ");}
...
else
{
    ...
    if ($v>=2) {warn("Added PSIPRED secondary structure to %i models\n",$nmodels);}
}    
...
sub AppendDsspSequences() {
	...
	if ($line=~/>(\S+)/) {
		...
		if ($v>=3) {warn("Warning: no pdb code found in sequence name '$name'\n");} 
		close(QFILE);
    ...
    close(QFILE);
    if ($v>=3) {warn("Searching DSSP state assignments: name=%s  range=%s\n",$name,$qrange);}

    # Try to open dssp file 
    $dsspfile="$dsspdir/$pdbcode.dssp";
    if (! open (DSSPFILE, "<$dsspfile")) {
	if ($v>=3) {warn( "Warning in $program: Cannot open $dsspfile!\n");} 
	...
	if (! open (DSSPFILE, "<$dsspfile")) {
	    if ($v>=3) {warn( "Warning in $program: dssp couldn't generate file from $pdbfile. Skipping $name\n");}
	    return 1;
	}
    }
...
#			print("qrange=$qrange  range='$range'  ires=$thisres  chain=$chain contained=$contained\n");
    ...
    if (length($aa_dssp)==0) {warn("WARNING: no residues found in $dsspdir/$pdbcode.dssp\n"); return 1;} 
    if (length($aa_dssp)<=20) {warn("WARNING: only %i residues found in $dsspdir/$pdbcode.dssp\n",length($aa_dssp)); return 1;} 
    ...
    # Debugging output
    if ($v>=4) {warn( "DSSP: %s: length=%-3i  score/len:%-5.3f\n",$name,$len,$score/$len);}
    if ($v>=4) {
	warn("IN:    %s\n",$xseq);
	warn("MATCH: %s\n",$Sstr);
	warn("DSSP:  %s\n",$yseq);
...
#    warn("aa=%s  sa=%5.1f  max_sa=%5.1f  rsa=%5.3f\n",$_[1],$_[0],$maxsa{$_[1]},$rsa);
...
sub OpenPDBfile() {
    ...
    my $pdbcode=lc($_[0]);
    if (! -e "$pdbdir") {
	if ($v>=3) {warn( "Warning in $program: pdb directory '$pdbdir' does not exist!\n");} 
	return 1;
    }
	...
	if ($v>=3) {warn( "Warning in $program: Cannot find pdb file $pdbfile"."pdb$pdbcode.ent!\n");}
	return "";
    }
    if (!open (PDBFILE, "$pdbfile")) {
	if ($v>=3) {warn( "Error in $program: Cannot open pdb file: $!\n");}
	return "";
    }
    return $pdbfile;
}
