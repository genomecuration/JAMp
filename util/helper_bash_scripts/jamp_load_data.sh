...
# load network data
cd-hit -c 0.95 -s 0.95 -M 0 -d 0 -T 8 -i $GENOME_GFF.pep -o $GENOME_GFF.pep.nr95
trim_fasta_all.pl -id $GENOME_GFF.pep.nr95 -i $GENOME_GFF.mRNA -inv -df -out $GENOME_GFF.mRNA.pepnr95
annotate_me.pl -annot_dbname $DBNAME -annot_port $PORT -annot_read $ROUSER -dataset_uname "$UNAME" \
 -annot_host $HOST -annot_user $RWUSER -annot_pass "$PASS" -authorization $AUTH \
 -network_description 'Similarity based clustering' -network_name "CDHIT 95% cluster" -network_type "cd-hit" -donetwork $GENOME_GFF.pep.nr95.clstr


makeblastdb -in $GENOME_GFF.pep -dbtype prot -parse_seqids -hash_index
blastp  -outfmt 6 -query $GENOME_GFF.pep -db $GENOME_GFF.pep -out $GENOME_GFF.pep.selfblast -evalue 1e-5 -num_threads 8
...
annotate_me.pl -annot_dbname $DBNAME -annot_port $PORT -annot_read $ROUSER -dataset_uname "$UNAME" \
 ...
 -network_name "MCL-BLAST" -network_type "MCL" -donetwork dump.out.$GENOME_GFF.pep.selfblast.I25
